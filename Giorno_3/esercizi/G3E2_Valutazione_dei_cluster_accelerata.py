...
# Collega order_items a customer_id tramite ordini
...
# 4. Stampa numero totale di clienti
print("Numero totale di clienti nel database:", len(agg))

# 5. Se più di 5000, riduci a un campione (random)
MAX_CLIENTI = 5000
if len(agg) > MAX_CLIENTI:
    agg = agg.sample(MAX_CLIENTI, random_state=42).reset_index(drop=True)
    print(f"Ridotto a un campione casuale di {MAX_CLIENTI} clienti.")

# Standardizza la feature per k-Means
X = StandardScaler().fit_transform(agg[['total_spent']])

# Clustering e silhouette su 2-7 cluster (velocissimo)
sil_scores = []
ks = range(2, 8)
for k in ks:
