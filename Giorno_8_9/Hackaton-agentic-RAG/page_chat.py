...
def is_memory_update_command(user_input: str) -> bool:
    ...
    update_patterns = [
        r'\b(ricorda|memorizza|annota) che\b',
        r"\b(da oggi|d'ora in poi)\b",
        r'\b(aggiorna|modifica) questa affermazione\b',
        r'\b(ti informo che|voglio che tu sappia)\b'
    ]
...
def display_chat_page(clients: RagClients, config: AppConfig):
    ...
    # Inizializzazione dello stato per la gestione di chat multiple
    ...
    # Gestione delle chat nella sidebar
    with st.sidebar:
    ...
    if prompt := st.chat_input("Scrivi qui la tua domanda o il tuo aggiornamento..."):
        ...
        # Assegna un nome alla chat se è la prima domanda
