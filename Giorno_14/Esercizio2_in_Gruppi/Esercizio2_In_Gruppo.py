...
def genera_risposta(question: str, context: str) -> str:
    qa = pipeline("question-answering", model="distilbert-base-uncased-distilled-squad")
    result = qa(question=question, context=context)
    return result["answer"]


def calcola_f1(prediction: str, reference: str) -> float:
...
def registra_feedback():
    rating = input("Valuta la risposta da 1 a 5 (invio per saltare): ")
    comment = ""
    if rating:
        comment = input("Commento (opzionale): ")
        feedback_history.append({"rating": int(rating), "comment": comment})
...
def main():
    ...
    print("\nTesto ripristinato:\n", testo_ripristinato)
    while True:
        domanda = input('Domanda (o "exit" per uscire): ')
        if domanda.lower() == 'exit':
            break
        risposta_attesa = input('Risposta attesa (opzionale): ')
        risposta = genera_risposta(domanda, testo_ripristinato)
        if feedback_history:
            media = sum(f['rating'] for f in feedback_history)/len(feedback_history)
            risposta += f'\n\n[Valutazione media precedente: {media:.2f}/5]'
        print('Risposta generata:', risposta)
        if risposta_attesa:
            score = calcola_f1(risposta, risposta_attesa)
