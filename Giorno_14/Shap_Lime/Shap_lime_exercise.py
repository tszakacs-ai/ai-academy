...
# Seleziona 5 email da spiegare
email_da_spiegare = nuove_email[:5]
predizioni = model.predict(vectorizer.transform(email_da_spiegare))

explainer = LimeTextExplainer(class_names=['ham', 'spam'])

# Funzione wrapper per LIME
def predict_proba_email(texts):
    return model.predict_proba(vectorizer.transform(texts))

print("\n--- Spiegazioni LIME per 5 email ---")
tabella = []
for i, email in enumerate(email_da_spiegare):
    exp = explainer.explain_instance(email, predict_proba_email, num_features=5)
    features = exp.as_list()
    top_features = ', '.join([f'"{feat}" ({"+ " if val > 0 else "- "}{abs(val):.2f})' for feat, val in features])
    tabella.append([f"Email {i+1}", predizioni[i], top_features])
    print(f"Email {i+1} | Predicted: {predizioni[i]} | Top Features: {top_features}")

import pandas as pd
df = pd.DataFrame(tabella, columns=["Record", "Predicted", "Top Features (LIME contribution)"])
print("\n", df.to_string(index=False))

print('Da questi risultati possiamo evincere che il modello si basa su segnali coerenti con la logica umana e non sembra ' \
      'mostrare bias evidenti nelle prime predizioni.')