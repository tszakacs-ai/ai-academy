...
from .AzureRag import AzureRAGSystem  # classe rag da file AzureRag.py
...
if st.sidebar.button("🔧 Inizializza RAG"):
    if st.session_state.uploaded_files:
        with st.spinner("🔍 Inizializzazione sistema RAG..."):
            ...
            # Aggiungi documenti al sistema RAG
...
if st.button("📤 Invia") or user_input:
    if user_input.strip():
        if st.session_state.pipeline is None:
            ...
            # Ricarica la pagina per mostrare la nuova conversazione
            st.rerun()
